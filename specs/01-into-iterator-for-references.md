...
- [ ] **T3 — Reference iteration is equivalent to `iter()` / `iter_mut()`, lint-clean, documented.** Needs: T1, T2. Scenarios: _Reference iteration matches iter() and iter_mut(); The clippy suppression is no longer needed_. Tests: property (proptest over `Some(any::<i32>())` / `Null` / `Absent`: `(&p).into_iter()` and `p.iter()` yield equal items with equal `len()` and `size_hint()`, item count is 1 iff `p.is_present()`; same for `&mut p` vs `iter_mut()`) in a `#[cfg(test)]` block in `src/presence.rs` + acceptance (the clippy scenario is verified by the lint gate, `keeler lint`, with no `iter_without_into_iter` expectation left in the source). Also updates the `Iter` / `IterMut` struct docs and adds `## [Unreleased]` → `### Added` to `CHANGELOG.md`.
